...
//删除学生信息
    ...
    @RequestMapping(value = "/coachFindList")
    public ResponseEntity<HashMap<String, Object>> coachFindList(int curr, int pageSize, String keywords, String coach ,HttpServletRequest request, HttpServletResponse response) throws IOException {
        response.setCharacterEncoding("UTF-8");
        response.setContentType("text/html;charset=utf-8");
        PrintWriter writer = response.getWriter();
        JSONObject jsonObject = new JSONObject();
        ...
        if (keywords.equals("")) {//如果关键字为空，则进行全局搜索
            List<GPStudent> studentList = gpStudnetService.selectCoachStudent(curr, pageSize,coach);
            System.out.println("listSize:" + studentList.size());
            if (!studentList.isEmpty()) {
                System.out.println("开始写入json");
//                jsonObject.put("count", gpStudnetService.selectStudentCount());
//                jsonObject.put("curr", curr);
            ...
            //存在关键字的情况
            List<GPStudent> studentList = gpStudnetService.selectCoachStudentByKeyword(keywords, curr, pageSize,coach);
            System.out.println("listSize:" + studentList.size());
            System.out.println("开始写入json");
            if (studentList.size() > pageSize) {
                int startIndex = (curr - 1) * pageSize;//开始截取的位置
